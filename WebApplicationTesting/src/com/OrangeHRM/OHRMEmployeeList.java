...
public class OHRMEmployeeList extends OHRMLoginTest {
	...
	public void loginOHRMTest() throws IOException {
		...
		/*
		 * // identifying userName element By userNameElementProperty =
		 * By.id(properties.getProperty("loginWebPageUserNameProperty"));
		 * 
		 * // finding userName element WebElement userNameElement =
		 * driver.findElement(userNameElementProperty);
		 * 
		 * // storing the value of user name into a variable String userId =
		 * properties.getProperty("UserName"); log.info("The user name is :- " +
		 * userId);
		 * 
		 ...
		 * type="password">
		 * 
		 * // locator/attribute - name // selector/value - txtPassword
		 * 
		 * // identifying password element By passwordElementProperty =
		 * By.name(properties.getProperty("loginWebPagePasswordProperty"));
		 * 
		 * // finding the password element (Text Box) WebElement passwordElement =
		 * driver.findElement(passwordElementProperty);
		 * 
		 * String passWord = properties.getProperty("Password");
		 * log.info("The password is :- " + passWord); // sending the keys through a
		 * variable to password element text box
		 * 
		 ...
		 * value="LOGIN">
		 * 
		 * // locator/attribute - className // selector/value - button
		 * 
		 * // identifying the button element by class name By buttonElementProperty =
		 * By.className(properties.getProperty("loginWebPageLoginButtonProperty"));
		 * 
		 * // finding the button element by class name WebElement buttonElement =
		 * driver.findElement(buttonElementProperty);
		 * 
	...
	public void printingEmployeeListTest() throws IOException {
		...
		for (int resultTableRowIndex = 1; resultTableRowIndex < resultTableRowCount; resultTableRowIndex++) {
			...
			XSSFRow newRow = testDataSheet.createRow(resultTableRowIndex);
	 ...
	 * By.linkText(properties.getProperty("loginWebPageWelcomeAdminElementProperty")
	 * );
	 * 
	 * // finding the welcome Admin Element
	 * 
	 * WebElement welcomeAdminElement =
	 * driver.findElement(welcomeAdminElementProperty);
	 * 
	 * // performing click operation on welcome Admin Element
	 * 
	 * welcomeAdminElement.click(); log.
	 * info("*****Successfully performed click operation on Welcome Admin Link*****"
	 * );
	 * 
	 * // Getting No such element exception in finding element logout // <a
	 * href="/OrangeHrm-4.2.0.1/symfony/web/index.php/auth/logout">Logout</a> //
	 * synchronization // Thread.sleep - java wait // we already have implicit wait
	 * which can be used by all web elements // but if we want to timeout for
	 * specific web element then we should go with // This is explicit wait time
	 * 
	 ...
	 * 
	 * By logoutElementLocator =
	 * By.linkText(properties.getProperty("loginWebPageLogoutElementProperty"));
	 * 
	 ...
	 * 
	 * logoutElement.click(); log.
	 * info("*******Successfully performed click operation on Logout Link*******");
	 * }
	 */
}